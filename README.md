# Lampolite
Lampolite is a lamp that runs when interacting with the touchpad for about 5.2 seconds 
# BOM ( Bill of material )
- 2 ICC timer NE555P
- 1 One expansion GIO pins 4017
- 20 LEDs
- 4 resistors
- 1 variable resistor
- 4 capacitors ( one is polarized )
- 6 pin headers : 3 for powering up the circuit and 3 for the TTP223 sensor
- The TTP223 Sensor
# PCB and schematics 
Here is the schematic 




<img width="756" height="666" alt="image" src="https://github.com/user-attachments/assets/30054a7c-35b6-40e8-b40e-a5ddcd0039fb" />





Here is the PCB :



<img width="625" height="606" alt="image" src="https://github.com/user-attachments/assets/699b7262-7978-4043-8f03-271fef5d7efe" />




Here is the 3D overview of the PCB:



<img width="841" height="636" alt="image" src="https://github.com/user-attachments/assets/75a85139-f61e-4568-a93e-fe1f0adfb014" />
 This is the project build :

 
 
 ![PHOTO2 (1)](https://github.com/user-attachments/assets/3a3c6497-a0a3-475e-be5a-d4d5ac04b7ca)


